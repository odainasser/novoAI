...
namespace Application.Features.PurchaseRequests;
...
{
    public PurchaseRequestSupplySource SupplySource { get; set; }
    /// <summary>The branch warehouse needing stock. For Branch Panel calls this is resolved server-side from the active branch.</summary>
    public Guid RequestingWarehouseId { get; set; }
    /// <summary>Optional supplier — only honoured when <see cref="SupplySource"/> is FromSupplier.</summary>
    public Guid? SupplierId { get; set; }
    public string? Notes { get; set; }
...
/// <summary>Body for the dedicated approve/reject endpoints. For reject, <see cref="Note"/> carries the reason.</summary>
