...
public class CreateOrderRequest
{
    ...
    // Optional idempotency key. When supplied, the server returns the already
    // created order instead of creating a duplicate — protects against offline
    // replays and double-submits. Offline clients set this to the local order id.
    public string? IdempotencyKey { get; set; }
    // UTC time the order was rung up on the client. Used by offline-replayed
    // orders so the server records the actual sale time, not the sync time.
    // Online clients leave this null; server falls back to DateTime.UtcNow.
